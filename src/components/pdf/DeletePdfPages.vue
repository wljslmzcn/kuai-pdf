...
import { deletePdfPages, copyFile, openFile, openFolder } from '../../api/pdf'
...
const handleDelete = async () => {
  ...
  try {
    const ts = Date.now()
    const tempPath = inputPath.value.replace(/\.pdf$/i, `_deleted_${ts}.pdf`)
    await deletePdfPages(inputPath.value, tempPath, pages.value)
...
const handleExport = async () => {
  if (!resultPath.value) {
    ElMessage.warning('请先点击"开始删除"生成预览')
    return
  }
...
<template>
  <div class="delete-pdf">
    ...
    <div class="form-section">
      ...
      <div class="hint">输入要删除的页码范围，多个范围用逗号分隔</div>
    </div>

    <div class="form-section button-group">
      <el-button type="primary" @click="handleDelete" :loading="loading" size="large">
        开始删除
      </el-button>
